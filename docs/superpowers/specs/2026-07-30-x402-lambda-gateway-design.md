...
For every request, Lambda derives `publicBaseUrl` only from the trusted REST
proxy `requestContext.domainName` and `requestContext.stage`. The domain must
match the generated execute-api hostname shape and the stage must be a safe
stage name; Lambda constructs the HTTPS URL and rejects missing or malformed
context as a safe configuration `503`. It never reads caller `Host` or
`X-Forwarded-*` values. Lambda has no public-base environment variable, which
removes the API-to-Lambda configuration dependency. The Agent retains its
separate `X402_GATEWAY_PUBLIC_BASE_URL` configuration and compares it to the
envelope value.
      ...
      "parts": [
        {
          ...
          "data": {
            ...
            "envelope": {
              ...
              "publicBaseUrl": "https://api-id.execute-api.us-east-1.amazonaws.com/testnet",
              "headers": {},
              "bodyBase64": ""
            }
          }
...
- `publicBaseUrl` is derived from trusted API Gateway REST
  `requestContext.domainName` and `requestContext.stage`, never copied from
  caller `Host` or forwarding headers. The domain must be the generated
  execute-api shape; construction is HTTPS-only and cannot contain credentials,
  query, or fragment. It must match the gateway base URL configured in the
  Agent Runtime. The internal dispatcher uses it when producing the x402
...
The trusted `publicBaseUrl` is constructed from API Gateway request context,
not forwarded request metadata.
...
2. AWS-managed common exploit rules in count-only mode where they do not interfere with the
   base64 payment header.

AWS WAF's body inspection cap is lower than the application's exact 256 KiB
request limit, so the Web ACL intentionally does not claim to enforce that
limit. Lambda and the Agent both retain the exact 256 KiB validation.
...
Lambda emits a root stdout JSON record in Lambda text log format with exactly
these fields: internal request ID, route, mapped status, upstream outcome
class, and duration. It does not emit embedded metrics or additional fields.
