...
- `publicBaseUrl` is injected from Lambda configuration, never copied from the
  caller's `Host` or forwarding headers. It must be HTTPS, contain no
  credentials, query, or fragment, and match the gateway base URL configured
  in the Agent Runtime. The internal dispatcher uses it when producing the
  x402 challenge `resource` URL.
...
The trusted `publicBaseUrl` is configuration, not forwarded request metadata.
