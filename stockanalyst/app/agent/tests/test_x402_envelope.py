...
class X402EnvelopeTests(unittest.IsolatedAsyncioTestCase):
    async def test_dispatch_preserves_payment_signature_transport(self) -> None:
        captured_scope = None
        proof = "opaque-b402-proof"
        ...
        self.assertIn(
            (b"payment-signature", proof.encode()),
    ...
    async def test_dispatch_rejects_query_on_paid_routes(self) -> None:
        with self.assertRaisesRegex(EnvelopeError, "query_not_allowed"):
            await dispatch_x402_envelope(
                recording_app,
                request_envelope(
                    method="GET",
